...
{
uint8_t i;
uint16_t addr = ReceivedDataBuffer[0x03] | (ReceivedDataBuffer[0x04]<<8);
...
TBLPTRU = 0;
TBLPTRH = ReceivedDataBuffer[0x04];
TBLPTRL = ReceivedDataBuffer[0x03];


for(i = 0; i < 32; ++i)
{
TABLAT = ReceivedDataBuffer[0x05+i];
asm ("tblwtpostinc");
}
...
{
uint16_t i;
uint8_t t_save[4] = {0x7E,0xEF,0x3F,0xF0};

for(i = 0;i<0x6840;i+=64)
{
PORTB |= 0b11100000;
TBLPTR = i;
EECON1bits.EEPGD = 1;
...
{
uint8_t i;
TBLPTR = ReceivedDataBuffer[0x03] | (ReceivedDataBuffer[0x04]<<8);

ToSendDataBuffer[0x04] = ReceivedDataBuffer[0x04];
ToSendDataBuffer[0x03] = ReceivedDataBuffer[0x03];

for(i=0;i<ReceivedDataBuffer[0x05];++i)
{
asm("TBLRD*+");
ToSendDataBuffer[0x05+i]=TABLAT;
}
}
